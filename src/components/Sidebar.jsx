...
export default function Sidebar({ 
  ...
  setIsDarkMode,
  closeMobileDrawer,
  isAdmin // 1. Accept the security boolean flag here
}) {
  const navItems = [
  ...
  return (
    <div className={`w-64 h-full p-6 flex flex-col justify-between border-r select-none transition-colors duration-300 ${
      isDarkMode ? 'bg-slate-900 border-slate-800 text-white' : 'bg-white border-gray-100 text-slate-900'
    }`}>
      {/* ... keeping the rest of your exact layout markup exactly the same ... */}
